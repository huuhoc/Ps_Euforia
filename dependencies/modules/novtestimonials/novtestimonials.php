...
class novtestimonials  extends Module
{
	...
	public function uninstall()
	{
		...
		return Configuration::deleteByName('novtestimonials_limit') && Configuration::deleteByName('novtestimonials_type') && parent::uninstall();
	}
	...
	private function _postValidation()
	{
		...
		elseif (Tools::isSubmit('submitTestimonials'))
		{
			...
			{
				if (Tools::strlen(Tools::getValue('content_'.$language['id_lang'])) > 255)
		...
		if (!$this->isCached('novtestimonials.tpl', $this->getCacheId()))
		{
            ...
            if(Tools::getValue('home')){
				...
				if(isset($config_home['novtestimonials_type']) && $config_home['novtestimonials_type']){
					...
					$config['novtestimonials_type'] = Configuration::get("novtestimonials_type") ? Configuration::get("novtestimonials_type") : 'type_one';
				}
	...
	public function renderForm()
	{
		...
		$list_style = array(
		  array(
			'value' => 'type_one',
			'name' 	=> 	$this->l('Type One')
		  ),
		  array(
			'value' => 'type_two',
			'name' 	=> 	$this->l('Type Two')
		  ),
		  array(
			'value' => 'type_three',
			'name' 	=> 	$this->l('Type Three')
		  ),
		);
		...
		$fields_form = array(
			'form' => array(
                ...
                'input' => array(
                	...
                	array(
					  'type' => 'select',                              
					  'label' => $this->l('Testimonials Item Style'),
					  'name' => 'novtestimonials_type',   
					  'default' => 'type_one',		                          
					  'options' => array(
						'query' => $list_style,
						'id' => 'value',                           
