...
if( !class_exists("Megahtml") ){
class Megahtml extends Megasource {
	public $type = 'html';
	public function renderForm($data){
			//$this->context->controller->addCss( 'https://fonts.googleapis.com/css?family=Open+Sans:400,700,800' ); 
			//$this->context->controller->addCss( __PS_BASE_URI__.'modules/'.$this->name.'/libs/css/pagemanage-item.css' );
			...
			$this->fields_form[0]['form'] = array(
				'tinymce' => true,
	            'legend' => array(
	                'title' => $this->l('Html Form.'),
	            ),
	            'input' => array(
					array(
	                    'type' => 'text',
	                    'label' => $this->l('Html Title'),
	                    'name' => 'title',
	                    'default'=> '',
						'required'=> true,
	                    'lang'	=> true
		...
		public function getDataSource( $config ){
			...
			$data['html'] = isset($config['html_'.$id_language])?($config['html_'.$id_language]): "";
