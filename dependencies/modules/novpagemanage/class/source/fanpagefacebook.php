...
if( !class_exists("Megafanpagefacebook") ){
class Megafanpagefacebook extends Megasource {
	public $type = 'fanpagefacebook';
	public function renderForm($data){
			...
			$this->fields_form[0]['form'] = array(
	            ...
	            'input' => array(
					...
					array(
	                    'type'  => 'text',
	                    'label' => $this->l('Fanpage Facebook Url'),
	                    'name'  => 'fanpage_url',
	                    'default'=> '#',
						'desc'  => $this->l('Example http://facebook.com/vinovathemes')
	                ),
					array(
						'type' => 'switch',
						'label' => $this->l('Hidden Cover'),
						'name' => 'hidden_cover',
						'values' => $active,
						'default' => 0,
					),
		...
		public function getDataSource( $config ){
			...
			$data['fanpage_url'] = isset($config['fanpage_url']) ? ($config['fanpage_url']) : 'http://facebook.com/vinovathemes';
			$data['hidden_cover'] = isset($config['hidden_cover']) ? ($config['hidden_cover']) : 0;
			$data['show_face'] = isset($config['show_face']) ? ($config['show_face']) : 1;
