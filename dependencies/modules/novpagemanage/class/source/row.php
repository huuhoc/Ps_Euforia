...
if( !class_exists("Megarow") ){
class Megarow extends Megasource {
	public $type = 'row';
	public function renderForm($data){
			...
			$delete_icon 	= Context::getContext()->link->getAdminLink('AdminModules', false) . '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name. '&type='. $this->type. '&hook='. $hook.'&id_novpagemanage='.$id_novpagemanage.'&removeIcon=1&token=' . Tools::getAdminTokenLite('AdminModules');
			$this->fields_form[0]['form'] = array(
				'tinymce' => true,
	            'legend' => array(
	                'title' => $this->l('Row Form.'),
	            ),
	            'input' => array(
					array(
	                    'type' => 'text',
	                    'label' => $this->l('Row Title'),
	                    'name' => 'title',
	                    'default'=> '',
						'required'=> true,
	                    'lang'	=> true
					...
					array(
						...
						'name' => 'image1',
						'desc' => $this->l('Backgroud image of row'),
						'image1' =>	$image,
					...
					array(
						...
						'desc' => 'HTML color only (e.g, "blue", "#000000")',
						'default'=> '',
					),
		...
		public function getDataSource( $config ){
			...
			$data['has_container'] = isset($config['has_container']) ? ($config['has_container']): 1;
			$data['columns'] = isset($config['columns']) ? ($config['columns']) : 12;
			$data['image1'] = isset($config['image1']) ? ($config['image1']) : '';
			$data['bg_color'] = isset($config['bg_color']) ? ($config['bg_color']) : '';
