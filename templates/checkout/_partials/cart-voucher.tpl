    ...
    <div class="block-promo">
      <div class="cart-voucher">
        ...
        <div class="promo-code collapse{if $cart.discounts|count > 0} in{/if}" id="promo-code">
          {block name='cart_voucher_form'}
            <form action="{$urls.pages.cart}" data-link-action="add-voucher" method="post">
              ...
              <input class="promo-input pl-3" type="text" name="discount_name" placeholder="{l s='Promo code' d='Shop.Theme.Checkout'}">
        ...
        {if $cart.discounts|count > 0}
          <p class="promo-highlighted promo-code-button mt-15">
            {l s='Take advantage of our exclusive offers:' d='Shop.Theme.Actions'}
          </p>
          <ul class="js-discount promo-discounts list-unstyled promo-code-button pt-0">
          {foreach from=$cart.discounts item=discount}
            <li>
              <span class="label"><span class="code">{$discount.code}</span> - <mark>{$discount.name}</mark></span>
            </li>
          {/foreach}
          </ul>
        {/if}
