			...
			<form class="std" method="post" action="{smartblog::GetSmartBlogLink('smartblog_search')}">
				<fieldset>
					<div>
						...
						<button class="btn btn-default button button-small" value="OK" name="smartblogsubmit" type="submit">
							<span>{l s="Ok" mod="smartblog"}</span>
						</button>
					</div>
				</fieldset>
			</form>
			<div class="buttons">
				<a title="Home" href="{smartblog::GetSmartBlogLink('smartblog')}" class="btn btn-default button button-medium">
					<span><i class="icon-chevron-left left"></i>{l s="Home page" mod="smartblog"}</span>
				</a>
			</div>
		</div>
	</div>
